...
describe('Create Admin API endpoints', () => {
  describe('POST /users/create_admin', () => {
    ...
    it('Should successfully create an admin a user', async () => {
      const response = await chai
        .request(app)
    ...
    it('Should return an error 500', () => {
      ...
      userController.adminCreateUser(request, response);
  ...
  describe('DELETE /users/:userId', () => {
    ...
    it('Should return an error 500', () => {
      ...
      const response = new Response();
      sinon.stub(response, 'status').returnsThis();
      userController.adminDeleteUser(request, response);
      expect(response.status).to.have.been.calledWith(500);
    });
  });
